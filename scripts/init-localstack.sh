...
RAW_BUCKET="${S3_RAW_BUCKET:-video-raw}"
ENCODED_BUCKET="${S3_ENCODED_BUCKET:-video-encoded}"
...
aws --endpoint-url="${ENDPOINT}" s3 mb "s3://${RAW_BUCKET}" 2>/dev/null || true
aws --endpoint-url="${ENDPOINT}" s3 mb "s3://${ENCODED_BUCKET}" 2>/dev/null || true
...
echo "S3 buckets: ${RAW_BUCKET}, ${ENCODED_BUCKET}"
...
aws --endpoint-url="${ENDPOINT}" s3api put-bucket-cors --bucket "${RAW_BUCKET}" --cors-configuration '{
  "CORSRules":[{"AllowedOrigins":["*"],"AllowedMethods":["GET","PUT","POST","HEAD"],"AllowedHeaders":["*"]}]
}' 2>/dev/null || true
aws --endpoint-url="${ENDPOINT}" s3api put-bucket-cors --bucket "${ENCODED_BUCKET}" --cors-configuration '{
